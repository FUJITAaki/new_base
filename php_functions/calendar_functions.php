...
function calendarCreate($count, $previewParam = null)
{
	...
	{	
		...
		$tag .= yobiTagCreate();
		$tag .= daysTagCreate($targetFirstDay, $targetLastDay);
		$tag .= "</div>\n";
	}
...
* 戻り値： HTMLタグ（日付全体）
****************************************************************/
function daysTagCreate($targetFirstDay, $targetLastDay)
{
...
* 戻り値： HTMLタグ（日付）
****************************************************************/
function dayTagCreate($targetDate)
{
	...
	if ($targetDate != "")
	{
		...
		{
			$tag .= linkTagCreate($targetDay, $linkInfoArray);
		}
	}
...
* 戻り値： HTMLタグ（リンク）
****************************************************************/
function linkTagCreate($targetDay, $linkInfoArray)
{
	global $classNameSuffix;
...
* 戻り値： 背景画像パターンコード
* 　　　　 1:休館日、2:特別展、3:絵はがき、4:イベント、5:特別展＆絵はがき、
* 　　　　 6:特別展＆イベント、7:行事無しの開館日
****************************************************************/
function backGroundPatternGet($targetDate, &$linkInfoArray)
{
	...
	if (count($linkInfoArray) == 1)
	{
		//当該日に行事情報が１つだけの時
		switch ($linkInfoArray[0]["gyoji_kubun"])
		{
			case 1: $code = 2; break;
			case 2: $code = 3; break;
			case 3: $code = 4; break;
	...
	else
	{
		...
		for ($i=0; $i < count($linkInfoArray); $i++)
		{
			if ($linkInfoArray[$i]["gyoji_kubun"] != 1)	//特別展は読み飛ばし
			{
				switch ($linkInfoArray[$i]["gyoji_kubun"])
				{
					case 2: $code = 5; break;
